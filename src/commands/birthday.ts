...
const execute = async (interaction: CommandInteraction) => {
    ...
    try {
        ...
        await interaction.reply({
            content: `${user}'s birthday is on **${format(
        ...
        if (
            ...
            collector.on("collect", async (i) => {
                if (i.customId === "birthdayPing") {
                    await user!.send({
                        content: `${interaction.user} wants you to add your birthday in **${interaction.guild}**! Use \`/setbirthday\` in **${interaction.guild}** to set it.`,
                    });
                    await i.update({
...
export default {
    data: new SlashCommandBuilder()
        ...
        .addUserOption((option) =>
            option
                .setName("user")
                .setDescription("The user whose birthday to get.")
        ),
    execute,
};
